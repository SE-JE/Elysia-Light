      ...
      if (controllers !== false) {
        if (!!controllers.length) {
          controllers.map(async (item: string) => {
            ...
            await controllerGeneration(struct.model, schema, relations, controller, route || controller)
          })
          ...
          await controllerGeneration(struct.model, schema, relations, struct.model, struct.model)
...
// ============================>
export async function modelGeneration(model: string, schema: Record<string, string> = {}, relations: Record<string, string> = {}): Promise<boolean> {
